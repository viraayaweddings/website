"""Compares worker-rendered venue pages against the static originals.
...
site-public directly and never invokes the worker:
    ...
    BASE=http://127.0.0.1:8803 python build/verify-hotel-render.py
...
for index, static_path in enumerate(pages, 1):
    parts = static_path.replace("\\", "/").split("/")
    city, slug = parts[-3], parts[-2]
    url = f"/destination-wedding/{city}/{slug}/"
...
print()
print(f"{len(pages) - len(failures)}/{len(pages)} venue pages render identically to the static originals")
