/**
 * Detaches the source markup from the data compiled into it.
 *
 * What it changes and why is in scripts/lib/page-data-transform.mjs, which
 * holds the transform itself. This is the pass over the two places on disk:
 *
 *   - site-public/**\/*.html, the originals build/verify-*-render.py compares
 *     rendered pages against;
 *   - worker/db/page-templates.generated.ts, the shells the home page and all
 *     259 venue pages are rebuilt from.
 *
 * The rows already in the database are a separate pass --
 * scripts/migrate-stored-pages.mjs -- because a deploy does not touch them.
 *
 *   node scripts/detach-hardcoded-data.mjs --check   report, change nothing
 *   node scripts/detach-hardcoded-data.mjs --apply
 */
...
for (const file of walk("site-public")) {
  ...
  if (!PAGE_DATA_MARKER.test(html)) continue;
...
const TEMPLATES = "worker/db/page-templates.generated.ts";
{
  ...
  const out = source.replace(/^(\s*html: )(".*")(,)$/gm, (whole, prefix, literal, suffix) => {
    ...
    shell += 1;
    if (!PAGE_DATA_MARKER.test(html)) return whole;
